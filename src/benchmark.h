...
// --- Global Constants ---
// Define constants used to configure the concurrent workload and key
// generation.
const size_t KEY_SIZE = 16;  // Fixed size (in bytes) for keys/values
const size_t MAX_KEY =
    1000000;  // Range of integer IDs used to generate keys (shared key space)
...
// The function executed by each thread, implementing the mixed put/get
// workload.
void ThreadBody(TreeMapBase& tree_map, size_t num_operations, double put_ratio,
...
int RunBenchmark(const int argc, char** argv) {
  if (argc != 5) {
    std::cerr << "Usage: " << argv[0]
              << " num_threads initial_size ops_per_thread put_ratio"
              << std::endl;
    return 1;
  }
  ...
  std::cout << "--- Pre-populating tree with " << initial_size
            << " elements... ---" << std::endl;
  ...
  const auto pre_populate_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(pre_populate_end -
                                                            pre_populate_start)
          .count();
  ...
  std::cout << "--- Starting Concurrent Mixed Workload (" << num_threads
            << " threads) ---" << std::endl;
  std::cout << "Total operations: " << total_operations << " ("
            << (put_ratio * 100) << "% PUTs, " << ((1.0 - put_ratio) * 100)
            << "% GETs)" << std::endl;
  ...
  for (size_t i = 0; i < num_threads; i++) {
    ...
    threads.emplace_back(std::thread(ThreadBody, std::ref(tree_map),
                                     ops_per_thread, put_ratio, thread_index,
  ...
  std::cout << "Concurrent computation took: " << total_ms << " ms"
            << std::endl;
  std::cout << "Total operations performed: " << total_operations_performed
            << std::endl;
  // Calculate Throughput (Operations/second)
  std::cout << "Throughput (Ops/sec): "
            << static_cast<double>(total_operations_performed) /
                   (total_ms / 1000.0)
            << std::endl;
